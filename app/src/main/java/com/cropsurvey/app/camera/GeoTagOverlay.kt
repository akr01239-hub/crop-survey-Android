 ...
 *  - VideoCaptureActivity (live preview overlay + PNG overlay burned into
 *    recorded video via FFmpeg post-processing)
...
object GeoTagOverlay {
     ...
     * sized to [width]. Used for live preview overlays (photo & video).
     */
    ...
    /**
     * Renders the stamp card as a standalone transparent-background bitmap
     * sized to fit a video frame of [frameWidth]x[frameHeight] - i.e. same
     * proportions as renderStampBitmap, but the caller (FFmpeg overlay) will
     * position it at the bottom of the frame. Used to generate the PNG
     * burned into recorded videos.
     */
    fun renderStampForVideo(frameWidth: Int, frameHeight: Int, coords: GpsCoords, placeName: String, survId: String): Bitmap {
        return renderStampBitmap(frameWidth, coords, placeName, survId)
    }
